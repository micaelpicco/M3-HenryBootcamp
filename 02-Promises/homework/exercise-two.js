...
function problemA() {
  ...
  // promise version
  const promStanza1 = promisifiedReadFile("poem-two/stanza-01.txt").then(
    (stanza1) => {
      blue(stanza1);
    }
  );

  const promStanza2 = promisifiedReadFile("poem-two/stanza-02.txt").then(
    (stanza2) => {
      blue(stanza2);
    }
  );

  Promise.all([promStanza1, promStanza2]).then(() => console.log("done"));
}
...
function problemB() {
  ...
  filenames = filenames.map((route) => {
    return promisifiedReadFile(route).then((texto) => blue(texto));
  });

  Promise.all(filenames).finally(() => console.log("done"));
}
...
function problemC() {
  ...
  filenames = filenames.map((route) => {
    return promisifiedReadFile(route);
  });

  Promise.all(filenames)
    .then((texto) => texto.forEach((texto) => blue(texto)))
