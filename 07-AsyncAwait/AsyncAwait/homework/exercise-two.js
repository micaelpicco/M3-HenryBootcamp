...
async function problemA() {
  ...
  // );
  // AsyncAwait version
  async function reading(filename) {
    blue(await promisifiedReadFile(filename));
  }
  // await reading('poem-two/stanza-01.txt');
  // await reading('poem-two/stanza-02.txt');
  // console.log('done');
  await Promise.all([
    reading('poem-two/stanza-01.txt'),
    reading('poem-two/stanza-02.txt'),
  ]);
  console.log('done');
}
...
async function problemB() {
  ...
  const arrayPromises = filenames.map((el) => reading(el));
